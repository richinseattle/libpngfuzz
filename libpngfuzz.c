...
unsigned int
crc32_ (unsigned int crc, unsigned char *buf, int len)
{
  unsigned char *end;
...
int pngPatch (unsigned char * png, size_t len) {
    if (len < 8)
        return -1;
    ...
    while (1) {
        ...
        uint32_t chunk_len = (png[i + 3]) | (png [i + 2] << 8) | (png[i + 1] << 16) | (png[i] << 24);
        printf(" chunk_len %d\n", chunk_len);

        if (chunk_len > 0x80000000)
            break;

        if (i + chunk_len + 12 > len)
            break;

        unsigned char * chunk = &png[i + 4];
        uint32_t crc = crc32_(0, chunk, chunk_len + 4);
        i = i + chunk_len + 8;

        uint32_t crc_found = (png[i + 3]) | (png[i + 2] << 8) | (png[i + 1] << 16) | (png[i] << 24);
        printf("checksums %08x %08x\n", crc_found, crc);
...
int main (int argc, char * argv []) {
  int retval = -1;
	...
	printf("main() offset: %llx\n", (char *)(&main) - (char *)GetModuleHandle(0));
